...
*/
/*****************************************************/
/* modifications by J.Schurig for inclusion in DPBOX */
/* deleted output procedures et al...		     */
/* if you are looking for a sane version, check	     */
/* http://www.top500.org			     */
/*****************************************************/
#define DP	/*				     */
#define UNROLL	/*				     */
...
static void dgefa (REAL a[],int lda,int n,int ipvt[],int *info);
...
void linpack(char *outs) /* was main */
{
	static REAL aa[200][200],a[200][201],b[200],x[200];
	REAL cray,ops,total,norma,normx;
	REAL resid,residn __attribute__((unused)),eps,t1,tm,tm2;
	REAL kf;
	...
	n = 100;
        ...
        matgen((RARR)a,lda,n,b,&norma);
        t1 = second();
        dgefa((RARR)a,lda,n,ipvt,&info);
        time[0][0] = second() - t1;
        t1 = second();
        dgesl((RARR)a,lda,n,ipvt,b,0);
        time[1][0] = second() - t1;
...
/*     compute a residual to verify results.  */ 
	...
	for (i = 0; i < ntimes; i++) {
        ...
        matgen((RARR)aa,ldaa,n,b,&norma);
        t1 = second();
        dgefa((RARR)aa,ldaa,n,ipvt,&info);
        time[0][4] = second() - t1;
        t1 = second();
        dgesl((RARR)aa,ldaa,n,ipvt,b,0);
        time[1][4] = second() - t1;
	...
	sprintf(outs,"%5d Kflops (%s%sPrecision)",kflops,ROLLING,PREC);
}
